...
apply_halfbroth_correction_unified <- function(data, od_column) {
  cat("  :abacus: Applying HALF-BROTH correction method (TIME 0 BASELINE):\n")
  cat("     Step 1: Calculate Half Broth baseline from time 0 values\n")
  cat("     Step 2: Apply Material Control (NP) correction to Experiment wells\n")
  cat("     Step 3: Apply Half Broth baseline correction to Positive Control wells\n")
  cat("     Using OD column:", od_column, "\n")
  # Create column names based on the detected OD column
  if (od_column == "od600_raw") {
    od_broth_corrected_column <- "od600_broth_corrected"
  ...
  # Step 1: Calculate Half Broth baseline from time 0 values (or earliest timepoint)
  halfbroth_data <- data %>%
    filter(sample_type == "half_broth")
  if (nrow(halfbroth_data) == 0) {
    cat("     :warning:  No Half Broth wells found! Cannot apply Half Broth correction.\n")
    halfbroth_correction_value <- 0
  } else {
    # Get the earliest timepoint (should be time 0 or very close)
    ...
    halfbroth_correction_value <- halfbroth_data %>%
      ...
      summarise(baseline = mean(.data[[od_column]], na.rm = TRUE)) %>%
      pull(baseline)
    cat("     :bar_chart: Half Broth baseline from time 0:", round(halfbroth_correction_value, 4), "\n")
    cat("     :bar_chart: Based on", sum(halfbroth_data$time_point == earliest_timepoint),
        "Half Broth wells at timepoint", earliest_timepoint, "\n")
  }
  # Step 2: Calculate time-matched NP corrections by CONCENTRATION (if NP controls exist)
  np_control_count <- sum(data$sample_type == "np_control", na.rm = TRUE)
  if (np_control_count > 0) {
    np_corrections_by_time <- data %>%
      filter(sample_type == "np_control") %>%
    ...
    cat("     :bar_chart: NP corrections calculated for",
        length(unique(np_corrections_by_time$concentration)), "concentrations\n")
  } else {
    cat("     :memo: No NP controls present - no NP correction for experiments\n")
  ...
  corrected_data <- data %>%
    ...
    mutate(
      ...
      !!od_final_column := case_when(
        # Experiment wells: subtract ONLY NP control
        ...
        # NP controls: no correction applied
        sample_type == "np_control" ~ .data[[od_column]],
        # Broth and Half Broth wells: no correction
        sample_type %in% c("broth", "half_broth") ~ .data[[od_column]],
        # Blanks: no correction
        sample_type == "blank" ~ .data[[od_column]],
        TRUE ~ .data[[od_column]]
      ),
      # Store correction components for diagnostics
      broth_correction_applied = ifelse(sample_type == "untreated_control",
                                        halfbroth_correction_value, 0),
  ...
  correction_stats <- corrected_data %>%
    ...
    summarise(
      ...
      mean_halfbroth_corr = mean(broth_correction_applied[sample_type == "untreated_control"], na.rm = TRUE),
      negative_values = sum(.data[[od_final_column]] <= 0, na.rm = TRUE),
      total_samples = n(),
      .groups = "drop"
    )
  cat("     :bar_chart: Final correction statistics:\n")
...
apply_halfbroth_correction_multi <- function(data, wavelength) {
  ...
  od_broth_corrected_column <- paste0("od", wavelength, "_broth_corrected")
  od_final_column <- paste0("od", wavelength, "_final")
  cat("  :abacus: Applying HALF-BROTH correction for", wavelength, "nm (TIME 0 BASELINE):\n")
  cat("     Step 1: Calculate Half Broth baseline from time 0 values\n")
  cat("     Step 2: Apply Material Control (NP) correction to Experiment wells\n")
  cat("     Step 3: Apply Half Broth baseline correction to Positive Control wells\n")
  ...
  if (nrow(halfbroth_data) == 0) {
    ...
    halfbroth_correction_value <- halfbroth_data %>%
      filter(time_point == earliest_timepoint) %>%
      summarise(baseline = mean(.data[[od_raw_column]], na.rm = TRUE)) %>%
      pull(baseline)
    cat("     :bar_chart: Half Broth baseline (", wavelength, "nm) from time 0:",
        round(halfbroth_correction_value, 4), "\n")
    cat("     :bar_chart: Based on", sum(halfbroth_data$time_point == earliest_timepoint),
        "Half Broth wells at timepoint", earliest_timepoint, "\n")
  }
  # First apply broth corrected column (placeholder)
  data <- data %>%
    mutate(
      !!od_broth_corrected_column := .data[[od_raw_column]]
    )
  # Step 2: Check if NP controls exist
  np_control_count <- sum(data$sample_type == "np_control", na.rm = TRUE)
  if (np_control_count == 0) {
    cat("     :memo: No NP controls present - no NP correction for experiments\n")
    # Apply only Half Broth correction to Positive Controls
    corrected_data <- data %>%
      mutate(
        !!od_final_column := case_when(
        ...
        broth_correction_applied = ifelse(sample_type == "untreated_control", halfbroth_correction_value, 0),
        np_correction_applied = 0,
        np_correction = 0
      )
  } else {
    cat("     :bar_chart: NP controls detected - calculating time-matched corrections\n")
    # Calculate time-matched NP corrections by CONCENTRATION
    np_corrections_by_time <- data %>%
      filter(sample_type == "np_control") %>%
      group_by(concentration, time_hrs, time_point) %>%
    ...
    corrected_data <- data %>%
      ...
      mutate(
        np_correction = ifelse(is.na(np_correction), 0, np_correction),
        !!od_final_column := case_when(
          # Experiment wells: subtract ONLY NP control (no broth subtraction!)
          sample_type == "sample" ~ pmax(.data[[od_raw_column]] - np_correction, 0),
          # Positive Control: subtract Half Broth baseline from time 0
          sample_type == "untreated_control" ~ pmax(.data[[od_raw_column]] - halfbroth_correction_value, 0),
  ...
  cat("     :bar_chart: Final correction statistics (", wavelength, "nm):\n")
  cat("        Half Broth baseline correction:", round(halfbroth_correction_value, 4), "\n")
  if (np_control_count > 0) {
    cat("        NP correction range: 0 to", round(correction_stats$max_np_corr, 4), "\n")
    cat("        Average NP correction:", round(correction_stats$mean_np_corr, 4), "\n")
...
calculate_replicate_statistics_unified <- function(data_fully_corrected, od_column) {
  ...
  # Only calculate stats for samples (not untreated controls)
  data_fully_corrected %>%
    filter(sample_type == "sample") %>%  # ONLY samples get replicate stats
    group_by(sample_type, concentration, time_hrs, time_point) %>%
    summarise(
      n_replicates = n(),
      mean_od = mean(.data[[od_final_column]], na.rm = TRUE),
...
plot_plate_heatmap <- function(corrected_results, timepoint = "last", 
  ...
  # Prepare data with improved negative value handling
  plot_data <- corrected_results$corrected_data %>%
    ...
    mutate(
      ...
      # For visualization: threshold very small negatives to a small positive value
      od_value = case_when(
        is.na(od_value_raw) ~ NA_real_,
        od_value_raw < 0 ~ 0.001,  # Small positive for negatives
        TRUE ~ pmax(od_value_raw, 0)
      ),
      # Create text labels as character strings - CHANGED TO 2 DECIMAL PLACES
      text_label = case_when(
        ...
        TRUE ~ as.character(round(od_value_raw, 2))  # CHANGED FROM 3 TO 2
      ),
      # Create text color based on value type
    ...
    group_by(time_point) %>%
    mutate(
      actual_time = round(mean(as.numeric(unlist(time_hrs)), na.rm = TRUE), 2),
      n_missing = sum(is.na(od_value_raw)),
      n_negative = sum(od_value_raw < 0, na.rm = TRUE),
      n_very_low = sum(od_value_raw >= 0 & od_value_raw < 0.001, na.rm = TRUE)
    ) %>%
    ungroup()
  
  # Create subtitle with issue summary
  subtitle_parts <- c(
    paste("Time:", plot_data$actual_time[1], "hours"),
    str_to_title(str_replace_all(data_type, "_", " "))
  )
  
  issue_parts <- c()
  if (plot_data$n_missing[1] > 0) {
    issue_parts <- c(issue_parts, paste(plot_data$n_missing[1], "missing"))
  }
  if (plot_data$n_negative[1] > 0) {
    issue_parts <- c(issue_parts, paste(plot_data$n_negative[1], "negative"))
  }
  if (plot_data$n_very_low[1] > 0) {
    issue_parts <- c(issue_parts, paste(plot_data$n_very_low[1], "very low"))
  }
  ...
  # Create heatmap with fixed text aesthetics
  p <- plot_data %>%
    ...
    scale_fill_viridis_c(name = "OD600", end = 0.9, option = "magma", 
                         na.value = "gray70") +
    scale_color_identity() +  # Use the colors we specified directly
    labs(
...
plot_plate_heatmap_multi <- function(corrected_results, timepoint = "last", 
  ...
  plot_data <- corrected_results$corrected_data %>%
    ...
    mutate(
      ...
      # Create text labels as character strings - 2 DECIMAL PLACES
      text_label = case_when(
        is.na(od_value_raw) ~ "NA",
        od_value_raw < -0.01 ~ paste0("(", round(od_value_raw, 2), ")"),
        od_value_raw < 0 ~ paste0("(-", abs(round(od_value_raw, 2)), ")"),
        od_value_raw < 0.001 ~ paste0("~", round(od_value_raw, 2)),
        TRUE ~ as.character(round(od_value_raw, 2))  # 2 DECIMAL PLACES
      ),
      # Create text color based on value type
      text_color = case_when(
        is.na(od_value_raw) ~ "red",
  ...
  # Create heatmap with matching aesthetics
  p <- plot_data %>%
    ggplot(aes(x = col_num, y = 9 - row_num)) +
    geom_tile(aes(fill = od_value), color = "white", linewidth = 0.5) +
    geom_text(aes(label = text_label, color = text_color), size = 2.2) +
...
create_qc_plots <- function(corrected_results, processed_data = NULL) {
  ...
  qc_plots <- list(
    broth_contamination = create_broth_contamination_plot(corrected_results),  # UPDATED
...
#' Create broth contamination detection plot
...
create_qc_plots_multi <- function(corrected_results, 
  ...
  qc_plots <- list(
    broth_contamination = create_broth_contamination_plot_multi(corrected_results, wavelength),  # UPDATED
...
#'
#' @export
show_plate_composite <- function(results, wavelength = NULL, include_layout = TRUE) {
  ...
  if ("wavelength_600" %in% names(results) || "wavelength_420" %in% names(results)) {
    ...
    if (is.null(wavelength)) {
      ...
      for (i in seq_along(wavelength_keys)) {
        ...
        # Check if combined_with_layout exists, if not create it on the fly
        if (include_layout && !"combined_with_layout" %in% names(results[[wl_key]]$plots$plate_composite)) {
          # Recreate the composite with layout
          updated_composite <- plot_plate_heatmap_composite_multi(
            results[[wl_key]]$corrected_results, 
            wl
          )
      ...
      if (include_layout && !"combined_with_layout" %in% names(results[[wavelength_key]]$plots$plate_composite)) {
        # Recreate the composite with layout
        updated_composite <- plot_plate_heatmap_composite_multi(
          results[[wavelength_key]]$corrected_results, 
          wavelength
        )
        composite_plot <- updated_composite$combined_with_layout
      } else if (include_layout) {
...
process_cerillo_kinetic_data <- function(kinetic_data, metadata) {
  ...
  for (idx in 4:min(10, length(all_cols))) {
    ...
    if (sum(!is.na(col_values)) > 0) {
      ...
      # Temperature should be between 20-50°C and relatively stable
  ...
  if (is.null(temp_col)) {
    ...
    cat("• Warning: No temperature column found, using default 37°C\n")
  }
...
setup_workspace("/home/william-ackerman/Desktop/Data RAW Ecoli Experiments")

list.files()
layout_ev_old <- launch_plate_editor()

r1 <- analyze_growth_curves(
  file_path = "25-07-21_E.coli_AMP_Day1_HN RAW.csv" ,
  layout_csv = layout_ev_old,
  orientation = "vertical"
)
show_qc_plots(r1)
show_growth_curves(r1)
verify_experimental_layout(r1) 
show_concentrations_panel(r1)
show_plate_composite(r1)

show_plate_initial_condition(r1)

r2 <- analyze_growth_curves(
  file_path = "25-07-21_E.coli_AMP_Day1_IT RAW.xlsx",
  layout_csv = layout_ev_old,
  orientation = "vertical"
)
show_qc_plots(r2)
show_growth_curves(r2)
verify_experimental_layout(r2) 
show_concentrations_panel(r2)
show_plate_composite(r2)


layout_ev_new <- launch_plate_editor()


r3 <- analyze_growth_curves(
  file_path = "25-07-31_Staph_Ag_sph_cube_Day1_KK RAW.xlsx",
  layout_csv = layout_ev_new,
  orientation = "vertical"
)
show_qc_plots(r3)
show_growth_curves(r3)
verify_experimental_layout(r3) 
show_concentrations_panel(r3)
r3$plots$plate_composite


r4 <- analyze_growth_curves(
  file_path = "25-07-22_E.coli_AMP_Day2_DP RAW.xlsx" ,
  layout_csv = layout_ev_new,
  orientation = "vertical"
)
show_qc_plots(r4)
show_growth_curves(r4)
verify_experimental_layout(r4) 
show_concentrations_panel(r4)
r4$plots$plate_composite
...
r5 <- analyze_growth_curves(
  file_path = "25-08-01_Entero_Ag_cube_Day4_JC RAW.csv",
  layout_csv = layout_ev_new,
  orientation = "vertical"
)
show_qc_plots(r5)
show_growth_curves(r5)
verify_experimental_layout(r5) 
show_concentrations_panel(r5)
show_plate_composite(r5)

formals(analyze_growth_curves)
r6 <- analyze_growth_curves(
  file_path = "25-08-01_Kleb_Ag_cube_Day4_DP RAW.csv",
  layout_csv = layout_ev_new,
  orientation = "vertical", 
  max_time_hrs =15
)
show_qc_plots(r6)
show_growth_curves(r6)
verify_experimental_layout(r6) 
show_concentrations_panel(r6)
show_plate_composite(r6)


list.files()
r7 <- analyze_growth_curves(
  file_path = "25-08-03_Entero_Ag_cube_Day6_KK_RAW.xlsx",
  layout_csv = layout_ev_new,
  orientation = "vertical"
)
show_qc_plots(r7)
show_growth_curves(r6)
verify_experimental_layout(r7) 
show_concentrations_panel(r7)
show_plate_composite(r7)

list.files() 

r8 <- analyze_growth_curves(
  file_path = "25-08-04_E.Coli_Ag_Cube_Day7_JC_RAW.xlsx" ,
  layout_csv = layout_ev_new,
  orientation = "vertical"
)
show_qc_plots(r8)
show_growth_curves(r8)
verify_experimental_layout(r8) 
show_concentrations_panel(r8)
show_plate_composite(r8)

r9 <- analyze_growth_curves(
  file_path = "25-08-04_E.Coli_Ag_Cube_Day7_JN_RAW.xlsx" ,
  layout_csv = layout_ev_new,
  orientation = "vertical"
)
show_qc_plots(r9)
show_growth_curves(r9)
verify_experimental_layout(r9) 
show_concentrations_panel(r9)
show_plate_composite(r9)

r10 <- analyze_growth_curves(
  file_path = "25-08-04_Staph_Ag_Cubes_Day7_DP_RAW.xlsx",
  layout_csv = layout_ev_new,
  orientation = "vertical"
)
show_qc_plots(r10)
show_growth_curves(r10)
verify_experimental_layout(r10) 
show_concentrations_panel(r10)
show_plate_composite(r10)


r11 <- analyze_growth_curves(
  file_path = "2025-08-04_entero_ag_cube_Day7.xlsx",
  layout_csv = layout_ev_new,
  orientation = "vertical"
)
show_qc_plots(r11)
show_growth_curves(r11)
verify_experimental_layout(r11) 
show_concentrations_panel(r11)
show_plate_composite(r11)


r12 <- analyze_growth_curves(
  file_path = "25-08-04_Kleb_Ag_cube_Day7_DP RAW.csv",
  layout_csv = layout_ev_new,
  orientation = "vertical"
)
show_qc_plots(r12)
show_growth_curves(r12)
verify_experimental_layout(r12) 
show_concentrations_panel(r12)
show_plate_composite(r12)


# Set working directory (adjust as needed)
setup_workspace("/home/william-ackerman/Desktop/Plate_Reader_Code")

# Launch plate editor
layout <- launch_plate_editor()

list.files()
formals(analyze_growth_curves)

results$plots$qc_plots$np_kinetics

# Cerillo plate test
results <- analyze_growth_curves(
  file_path = "25-07-16_Enterobacter_AgNO3_Day1_IT RAW.csv",
  layout_csv = layout,
  orientation = "vertical"
)
...
verify_experimental_layout(results) 
show_concentrations_panel(results)
results$plots$plate_composite
results$plots$qc_plots$np_kinetics

# Synergy plate test
res2 <- analyze_growth_curves(
  file_path = "25-07-16_Enterobacter_AgNP cube_Day1_JN RAW.xlsx" ,
  orientation = "vertical", 
  correction_method = "standard"
)
show_qc_plots(res2, wavelength = "600")
show_growth_curves(res2, wavelength = "600")
verify_experimental_layout(res2$wavelength_600) 
show_concentrations_panel(res2, wavelength = "600")
res2$wavelength_600$plots$plate_composite

res3 <- analyze_growth_curves(
  file_path = "25-07-17_Enterobacter_AgNP cube_Day2_DP RAW.xlsx" ,
  orientation = "vertical", 
  correction_method = "standard"
)
show_qc_plots(res3, wavelength = "600")
show_growth_curves(res3, wavelength = "600")
verify_experimental_layout(res3$wavelength_600) 
show_concentrations_panel(res3, wavelength = "600")
res3$wavelength_600$plots$plate_composite

res4 <- analyze_growth_curves(
  file_path = "25-07-16_Enterobacter_AgNP sphere_Day1_IT RAW.csv",
  orientation = "vertical", 
  correction_method = "standard"
)
show_qc_plots(res4, wavelength = "600")
show_growth_curves(res4, wavelength = "600")
verify_experimental_layout(res4) 
show_concentrations_panel(res4)
res3$wavelength_600$plots$plate_composite


res5 <- analyze_growth_curves(
  file_path = "25-07-17_Enterobacter_AgNP sphere_Day2_JC RAW.csv",
  orientation = "vertical", 
  correction_method = "standard"
) 
show_qc_plots(res5, wavelength = "600")
show_growth_curves(res5, wavelength = "600")
verify_experimental_layout(res5) 
show_concentrations_panel(res5)
res5$plots$plate_composite
