...
plot_plate_heatmap_multi <- function(corrected_results, timepoint = "last", 
  ...
  if (timepoint == "first") {
    ...
    # CHANGED: Find the last timepoint that has data for ALL rows
    timepoint_coverage <- corrected_results$corrected_data %>%
      group_by(time_point) %>%
      summarise(
        n_rows = n_distinct(row_letter),
        .groups = "drop"
      ) %>%
      filter(n_rows == 8)  # All 8 rows must be present
    
    if (nrow(timepoint_coverage) > 0) {
      target_timepoint <- max(timepoint_coverage$time_point)
      cat("  • Using timepoint", target_timepoint, "as 'last' (latest with all rows)\n")
    } else {
      # Fallback to max timepoint
      target_timepoint <- max(corrected_results$corrected_data$time_point)
      cat("  • WARNING: No timepoint has all rows. Using max timepoint", target_timepoint, "\n")
    }
    time_label <- "Final"
  } else {
    target_timepoint <- timepoint
  ...
  # First, create a complete grid of all wells
  ...
  # Join with complete grid to ensure all wells are represented
  plot_data <- complete_wells %>%
    left_join(timepoint_data, by = "well_id") %>%
    mutate(
      od_value_raw = .data[[od_column]],
      # For missing wells, use NA
      od_value = case_when(
        is.na(od_value_raw) ~ NA_real_,
        od_value_raw < 0 ~ 0.001,  # Small positive for negatives
        TRUE ~ pmax(od_value_raw, 0)
...
r1 <- analyze_growth_curves(
  file_path = "25-08-06_Entero_Ag_cube_Day9_JC RAW.xlsx",
