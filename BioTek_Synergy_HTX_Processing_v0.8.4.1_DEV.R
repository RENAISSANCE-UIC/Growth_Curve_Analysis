...
apply_halfbroth_correction_unified <- function(data, od_column) {
  ...
  cat("  🧮 Applying HALF-BROTH correction method:\n")
  cat("     Step 1: Detect contamination in Broth and Half Broth wells\n")
  cat("     Step 2: Apply Material Control (NP) correction to Experiment wells\n") 
  cat("     Step 3: Apply Half Broth correction to Positive Control wells\n")
  cat("     Using OD column:", od_column, "\n")
  ...
  # Step 1: Detect contamination in BOTH Broth and Half Broth wells
  contamination_check <- data %>%
  ...
  # Identify contaminated wells
  median_mean_od <- median(contamination_check$mean_od, na.rm = TRUE)
  mad_mean_od <- mad(contamination_check$mean_od, na.rm = TRUE)
  median_range <- median(contamination_check$od_range, na.rm = TRUE)
  mad_range <- mad(contamination_check$od_range, na.rm = TRUE)
  
  contaminated_wells <- contamination_check %>%
    filter(
      (mean_od > median_mean_od + 3 * mad_mean_od) |
        (od_range > median_range + 3 * mad_range) |
        (growth_trend > 0.3)
  ...
  if (length(contaminated_wells) > 0) {
    cat("     ⚠️  Contaminated wells detected:", paste(contaminated_wells, collapse = ", "), "\n")
  } else {
    cat("     ✓ No contaminated wells detected\n")
  }
  
  # Step 2: Calculate Half Broth correction value (for Positive Controls)
  clean_halfbroth_data <- data %>%
    filter(sample_type == "half_broth", !well_id %in% contaminated_wells)
  
  if (nrow(clean_halfbroth_data) == 0) {
    cat("     ⚠️  No clean Half Broth wells found! Using minimal correction.\n")
    ...
    cat("     📊 Half Broth correction using", length(unique(clean_halfbroth_data$well_id)), 
        "clean wells:", round(halfbroth_correction_value, 4), "\n")
  }
  ...
  corrected_data <- data %>%
    ...
    mutate(
      ...
      !!od_final_column := case_when(
        ...
        # Positive Control (Untreated Bacteria): subtract Half Broth
  ...
  cat("     📊 Final correction statistics:\n")
  cat("        Half Broth correction (for Positive Controls):", round(halfbroth_correction_value, 4), "\n")
  cat("        NP correction range: 0 to", round(correction_stats$max_np_corr, 4), "\n")
  cat("        Average NP correction:", round(correction_stats$mean_np_corr, 4), "\n")
  cat("        Negative values after correction:", correction_stats$negative_values, 
      "out of", correction_stats$total_samples, "\n")
...
apply_halfbroth_correction_multi <- function(data, wavelength) {
  ...
  cat("  🧮 Applying HALF-BROTH correction for", wavelength, "nm:\n")
  cat("     Step 1: Detect contamination in Broth and Half Broth wells\n")
  cat("     Step 2: Apply Material Control (NP) correction to Experiment wells\n") 
  cat("     Step 3: Apply Half Broth correction to Positive Control wells\n")
  ...
  if (length(contaminated_wells) > 0) {
    cat("     ⚠️  Contaminated wells detected (", wavelength, "nm):", 
        paste(contaminated_wells, collapse = ", "), "\n")
  } else {
    cat("     ✓ No contaminated wells detected (", wavelength, "nm)\n")
  }
  
  # Step 2: Calculate Half Broth correction value
  clean_halfbroth_data <- data %>%
    filter(sample_type == "half_broth", !well_id %in% contaminated_wells)
  ...
  if (nrow(clean_halfbroth_data) == 0) {
    ...
    cat("     📊 Half Broth correction (", wavelength, "nm) using", 
        length(unique(clean_halfbroth_data$well_id)), "clean wells:", 
  ...
  if (np_control_count == 0) {
    cat("     📝 No NP controls present - no NP correction for experiments\n")
    ...
    corrected_data <- data %>%
      ...
      mutate(
        ...
        !!od_final_column := case_when(
          ...
          # Positive Control: subtract Half Broth
          sample_type == "untreated_control" ~ pmax(.data[[od_raw_column]] - halfbroth_correction_value, 0),
...
export_wide_format <- function(results, data_type = "corrected", 
  ...
  # Retry logic for data access (sometimes there are transient issues)
  data <- NULL
  ...
  # Detect the OD column based on data type with better error handling
  od_column <- tryCatch({
  ...
  # Get the data for export with validation
  plot_data <- data %>%
    filter(sample_type %in% sample_types) %>%
  ...
  # Get unique time points and concentrations with validation
  ...
  # Add AVERAGE columns with error handling
  for (conc in conc_sorted) {
    tryCatch({
      avg_data <- plot_data %>%
      ...
      cat("• Warning: Could not calculate average for concentration", conc, ":", e$message, "\n")
    })
  }
  ...
  if (length(replicate_ids) > 1) {
    for (conc in conc_sorted) {
      tryCatch({
        ...
        cat("• Warning: Could not calculate SD for concentration", conc, ":", e$message, "\n")
      })
    }
  } else {
