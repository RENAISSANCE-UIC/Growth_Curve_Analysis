...
create_ordinal_concentrations <- function(layout_data) {
  ...
  sample_concentrations <- layout_data %>%
    filter(condition %in% c("SAMPLE", "NP_only"), !is.na(concentration_value)) %>%
...
setup_workspace("/home/weackerm/Desktop/Plate_Reader_Code")
...
results_vertical <- analyze_growth_curves(
  file_path = "25-07-08_E coli_AgNP_56nm_JC RAW.csv",
  layout_csv = NULL,  # Use default layout
  orientation = "vertical",  # NEW parameter
  correction_method = "standard"
)
show_qc_plots(results_vertical)
