  ...
  if (!is.null(layout_csv)) {
    ...
    layout <- layout_data %>%
      ...
      select(well_id, condition, sample_type, concentration, replicate_id, 
             row_letter, col_number, category, concentration_value, concentration_units)
    ...
    # Use default hardcoded layout (existing code)
    cat("📋 Using default experimental layout\n")
    layout <- create_default_layout()
    layout_summary <- create_default_summary(layout)
  }
...
process_with_corrections <- function(processed_data, method = "traditional", threshold = TRUE) {
...
calculate_broth_correction <- function(data, method = "traditional") {
  ...
  if (method == "traditional") {
    # ... existing traditional code ...
  } else if (method == "robust") {
    # ... existing robust code ...
...
calculate_broth_correction_multi <- function(data, method = "traditional", wavelength) {
...
calculate_np_corrections <- function(data_broth_corrected, method = "traditional") {
...
analyze_growth_curves <- function(file_path, layout_type = "default", 
  ...
  suppressWarnings({
    ...
    if (!is.null(layout_csv)) {
      ...
      cat("📋 Using custom plate layout from:", basename(layout_csv), "\n")
    }
    
    # Process with custom or default layout
    multi_results <- analyze_growth_curves_multi(file_path, layout_type, 
                                                 correction_method, threshold, 
                                                 layout_csv)  # Pass layout file
...
results <- analyze_growth_curves("25-06-17_AgNP_cubes_DP_raw_plate_layout.xlsx",
                                 correction_method = "standard")
results$wavelength_600$plots$qc_plots$before_after
