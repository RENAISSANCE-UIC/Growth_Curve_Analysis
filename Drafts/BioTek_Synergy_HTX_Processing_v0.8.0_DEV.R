...
create_ordinal_concentrations <- function(layout_data) {
  ...
  conc_mapping <- sample_concentrations %>%
    mutate(conc_label = paste0("Conc_", conc_rank)) %>%
...
process_synergy_file <- 
  function(file_path, sheet_name = NULL, layout_type = "default", 
           layout_csv = NULL, orientation = "horizontal") {
  ...
  corrected_data <- data %>%
    ...
    mutate(
      !!od_final_column := case_when(
        ...
        sample_type %in% c("broth", "blank") ~ {
          # NEW: Apply broth correction to broth and blank wells too
          pmax(.data[[od_raw_column]] - broth_correction_value, 0)
        },
...
verify_experimental_layout <- function(results) {
  ...
  if ("processed_data" %in% names(results)) {
    ...
    # For the current structure, we need to create a processed_data-like object
    processed_data <- list(
      corrected_data = results$corrected_results$corrected_data,
      layout = results$processed_data$layout
    )
  } else {
    stop("Cannot find data in results object")
  }
  ...
  for (stype in c("blank", "sample", "np_control", "untreated_control")) {
    ...
    if (nrow(wells_of_type) > 0) {
      ...
      } else if (stype == "untreated_control") {
        # For untreated controls, just show the wells
        cat("Wells:", paste(wells_of_type$well_id, collapse = ", "), "\n")
      } else {
  ...
  replicate_check <- layout_data %>%
    filter(sample_type == "sample") %>%  # Only check samples for replicates
  ...
  # Check for uneven replicates (only for samples)
  uneven_reps <- replicate_check %>%
    filter(n_replicates != 5)  # Based on your layout, expecting 5 replicates for samples
  ...
  return(list(
    ...
    untreated_summary = layout_validation$untreated
  ))
}
...
create_broth_contamination_plot <- function(corrected_results) {
  ...
  if (nrow(broth_data) == 0) {
    return(ggplot() + 
             labs(title = "QC: No Broth Controls Found") + 
             theme_minimal())
  }
  ...
  broth_summary <- broth_data %>%
    group_by(well_id) %>%
    summarise(
      initial_od = first(.data[[od_raw_column]]),
      final_od = last(.data[[od_raw_column]]),
      max_od = max(.data[[od_raw_column]], na.rm = TRUE),
  ...
  broth_data %>%
    ...
    labs(
      title = "QC: Broth Control Contamination Detection",
      subtitle = "Monitoring broth-only wells for signs of bacterial growth (contamination)",
      x = "Time (hours)",
      y = "Raw OD600",
...
create_broth_contamination_plot_multi <- function(corrected_results, wavelength) {
  ...
  broth_data <- corrected_results$corrected_data %>%
    filter(sample_type == "broth")
  
  if (nrow(broth_data) == 0) {
    return(ggplot() + 
             labs(title = paste("QC: No Broth Controls Found -", wavelength, "nm")) + 
             theme_minimal())
  }
  ...
  broth_data %>%
    ...
    labs(
      title = paste("QC: Broth Contamination Detection -", wavelength, "nm"),
      subtitle = "Monitoring broth-only wells for bacterial growth (contamination)",
      x = "Time (hours)",
  ...
  suppressWarnings({
    ...
    multi_results <- 
      analyze_growth_curves_multi(file_path, layout_type, layout_csv,
                                  correction_method, threshold, orientation)  # ADDED orientation
