...
create_experiment_layout <- function(layout_type = "default", layout_csv = NULL) {
  ...
  if (!is.null(layout_csv)) {
    # Read layout from Shiny app export
    ...
    # Use default hardcoded layout (FIXED - this is the original working code)
...
#' Launch plate editor UI
launch_plate_editor <- function() {
  ...
  cat("2. Download the CSV file\n") 
  cat("3. Use the CSV with: analyze_growth_curves(data_file, layout_csv = 'your_layout.csv')\n\n")
...
process_custom_layout <- function(layout_csv) {
  ...
  # Convert to the expected format with well ID standardization
  layout <- layout_data %>%
    mutate(
    ...
    arrange(well_id) %>%  # Ensure consistent ordering
    group_by(condition) %>%
    mutate(
      # POSITIONAL CONCENTRATION ASSIGNMENTS
      concentration = case_when(
        # For samples and NP controls: use column position (like original)
        condition %in% c("SAMPLE", "NP_only") ~ paste0("Conc_", col_number - 1),
        
        # For untreated controls: sequential numbering Conc_1, Conc_2, ..., Conc_X
        condition == "UNT" ~ paste0("Conc_", row_number()),
        
        # For blanks and broth: use position-based naming
    ...
    mutate(
      ...
      replicate_id = case_when(
        # For samples and untreated controls, use the replicate label from CSV
        !is.na(replicate_label) ~ str_replace_all(replicate_label, " ", "_"),
        
        # For NP controls
        sample_type == "np_control" ~ paste0("NP_", row_letter),
        
        # For others
        sample_type == "blank" ~ paste0("Blank_", row_letter),
        sample_type == "broth" ~ paste0("Broth_", row_letter),
        TRUE ~ NA_character_
      )
    ) %>%
    select(well_id, condition, sample_type, concentration, replicate_id, 
  ...
  # Show the concentration assignments to verify they're correct
  ...
  # Show untreated control assignments specifically
...
process_synergy_file_multi <- function(file_path, sheet_name = NULL, layout_type = "default", layout_csv = NULL) {
...
process_with_corrections <- function(processed_data, method = "standard", threshold = TRUE) {
  ...
  if (method == "standard") {
    ...
    data_fully_corrected <- apply_standard_correction(data)
...
calculate_broth_correction_multi <- function(data, method = "standard", wavelength) {
  ...
  if (method == "traditional") {
    ...
    # For standard/statistician methods, correction is applied differently
    cat("  📊", str_to_title(method), "broth correction (", wavelength, "nm): Applied in main correction function\n")
    broth_correction <- 0
...
calculate_np_corrections <- function(data_broth_corrected, method = "standard") {
  if (method == "traditional") {
    ...
    cat("  🔬 Standard NP correction: Applied in main correction function\n")
...
calculate_np_corrections_multi <- 
  ...
  if (method == "traditional") {
    ...
    np_corrections <- data_broth_corrected %>%
      filter(sample_type == "np_control") %>%
      select(time_hrs, time_point, !!od_raw_column, well_id, col_number) %>%
      mutate(
        concentration = paste0("Conc_", col_number - 1)
      ) %>%
    ...
    np_corrections <- data_broth_corrected %>%
      filter(sample_type == "np_control") %>%
      select(time_hrs, time_point, concentration, !!od_raw_column) %>%
      group_by(time_hrs, time_point, concentration) %>%
      summarise(np_correction = mean(.data[[od_raw_column]], na.rm = TRUE), .groups = "drop")
    
  } else if (method %in% c("standard")) {
    # For standard/statistician methods, NP correction is applied differently
    cat("  🔬", str_to_title(method), "NP correction (", wavelength, "nm): Applied in main correction function\n")
...
apply_standard_correction <- function(data, wavelength = NULL) {
  ...
  # Step 3: Calculate time-matched NP corrections by column
  np_corrections_by_time <- data %>%
    filter(sample_type == "np_control") %>%
    group_by(col_number, time_hrs, time_point) %>%
    summarise(
  ...
  corrected_data <- data %>%
    ...
    # Join NP corrections by column and time
    left_join(np_corrections_by_time, by = c("col_number", "time_hrs", "time_point")) %>%
...
apply_standard_correction_multi <- function(data, wavelength) {
  ...
  # Step 3: Calculate time-matched NP corrections
  np_corrections_by_time <- data %>%
    filter(sample_type == "np_control") %>%
    group_by(col_number, time_hrs, time_point) %>%
    summarise(
...
analyze_growth_curves_multi <- function(file_path, layout_type = "default", layout_csv = NULL,
  ...
  processed_data <- process_synergy_file_multi(file_path, layout_type = layout_type, layout_csv = layout_csv)
...
analyze_growth_curves <- function(file_path, layout_type = "default", 
  ...
  suppressWarnings({
    ...
    # Use multi-wavelength analysis with layout_csv parameter
    ...
    if (n_wavelengths == 1) {
      ...
      # NO NEED TO APPLY CUSTOM LAYOUT HERE AGAIN - it's already applied!
...
run_analysis <- function(file_path, export_summaries = TRUE, 
  ...
  if (is.null(threshold)) {
    threshold <- switch(method,
                        "traditional" = FALSE,    # Original colleague method - no thresholding
                        "robust" = TRUE,          # Robust method - use thresholding for safety
                        "standard" = TRUE      # Standard method - use thresholding for safety  
    )
  }
...
#' @rdname run_analysis
#' @export
run_traditional_analysis <- function(file_path, export_summaries = TRUE) {
  run_analysis(file_path, export_summaries, method = "traditional", threshold = FALSE)
}

#' @rdname run_analysis  
#' @export
run_robust_analysis <- function(file_path, export_summaries = TRUE) {
  run_analysis(file_path, export_summaries, method = "robust", threshold = TRUE)
}

#' @rdname run_analysis
#' @export  
run_standard_analysis <- function(file_path, export_summaries = TRUE) {
  run_analysis(file_path, export_summaries, method = "standard", threshold = TRUE)
}
...
run_custom_analysis <- function(data_file, layout_file = NULL, 
  ...
  if (is.null(layout_file)) {
    ...
    results <- run_analysis(data_file, export, method)
  } else {
...
getwd()
list.files(pattern = "my_plate_layout_96well_2025-005-08_Ag_sph_50.csv")
list.files(pattern = "25-05-08_MG1655_Ag_sphere_50_10_PVP RAW.xlsx")

results <- analyze_growth_curves(
  file_path = "25-05-08_MG1655_Ag_sphere_50_10_PVP RAW.xlsx",
  layout_csv = "my_plate_layout_96well_2025-005-08_Ag_sph_50.csv",
  correction_method = "standard",
...
results$plots$concentrations_panel
results$plots$qc_plots$layout_validation

layout_preview <- read_csv("my_plate_layout_96well_2025-005-08_Ag_sph_50.csv")
head(layout_preview)
names(layout_preview)
print(results$plots$qc_plots$layout_validation)
# Verify layout validation
validate_layout_file("my_plate_layout_96well_2025-005-08_Ag_sph_50.csv")
...
show_qc_plots(results)
plot_concentrations_panel(results)
...
results <- analyze_growth_curves_standard("25-06-17_AgNP_cubes_DP_raw_plate_layout.xlsx")
results$wavelength_600$plots$concentrations_panel

# Or specify directly
results <- analyze_growth_curves("25-06-17_AgNP_cubes_DP_raw_plate_layout.xlsx",
                                 correction_method = "standard")

run_analysis("25-06-17_AgNP_cubes_DP_raw_plate_layout.xlsx")

launch_plate_editor()
results <- analyze_growth_curves("25-06-17_AgNP_cubes_DP_raw_plate_layout.xlsx",
                                 layout_csv = "plate_layout_96well_2025-07-07.csv")

validate_layout_file("plate_layout_96well_2025-07-07.csv")
results <- analyze_with_custom_layout("25-06-17_AgNP_cubes_DP_raw_plate_layout.xlsx",
                                      "plate_layout_96well_2025-07-07.csv")
results$wavelength_600$correction_method
results$wavelength_600$threshold
results$wavelength_600$plots$concentrations_panel

results$wavelength_600$corrected_results$corrected_data
