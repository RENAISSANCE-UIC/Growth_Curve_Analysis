...
get_replicate_stats <- function(results) {
  results$corrected_results$replicate_stats
}
...
#' All-in-one results explorer
explore_results <- function(results) {
  
  cat("• EXPLORING YOUR RESULTS\n")
  cat(strrep("=", 40), "\n\n")
  ...
  if ("summaries" %in% names(results)) {
    ...
    cat("\n")
    show_growth_ranking(results, top_n = 5)
    cat("\n")
    show_quality_summary(results)
  } else {
    cat("• For detailed summaries, run analyze_growth_curves() with export_summaries = TRUE\n")
  }
  
  cat("\n• Available functions: show_available_functions()\n")
}
...
analyze_growth_curves_multi <- function(file_path, layout_type = "default") {
  ...
  for (wavelength_name in names(processed_data$wavelength_data)) {
    ...
    plots <- list(
      ...
      concentrations_panel = plot_concentrations_panel_single(corrected_results, NULL, "sample", "corrected", FALSE, wavelength),  # NEW!
