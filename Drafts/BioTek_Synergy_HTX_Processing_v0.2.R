...
plot_multiple_concentrations <- function(results, concentrations,
                                         sample_type = "sample",
                                         data_type = "corrected") {
  
  # Get data for all requested concentrations
  plot_data <- map_dfr(concentrations, ~{
  ...
  # Create the plot
  p <- plot_data %>%
    ggplot(aes(x = time_hrs, y = od600, color = concentration)) +
    geom_line(aes(group = interaction(concentration, replicate_id)), 
              size = 1, alpha = 0.7) +
    geom_point(aes(shape = replicate_id), size = 2, alpha = 0.8) +
    scale_color_viridis_d(name = "Concentration") +
    scale_shape_manual(values = c("Rep_A" = 16, "Rep_B" = 17, "Rep_C" = 15, "Rep_UNT" = 18),
                       name = "Replicate") +
    labs(
      title = "Growth Curves - Multiple Concentrations",
      subtitle = paste("Sample type:", str_to_title(str_replace_all(sample_type, "_", " ")), 
                       "|", str_to_title(str_replace_all(data_type, "_", " "))),
      x = "Time (hours)",
      y = "OD600"
    ) +
...
check_replicate_structure(full_results)
colleague_data <- get_colleague_format(full_results, include_untreated = TRUE)
# Test the fixed version
colleague_data <- get_colleague_format(full_results, include_untreated = TRUE)

# Browse available concentrations
browse_concentrations(full_results)

# Get data for a specific concentration
conc5_data <- get_concentration_data(full_results, "Conc_5")
head(conc5_data)

# Get it in wide format
conc5_long <- get_concentration_data(full_results, "Conc_5", format = "long")
head(conc5_long)

# Get multiple concentrations at once
high_concs <- get_multiple_concentrations(full_results, c("Conc_8", "Conc_9", "Conc_10"))
names(high_concs)

# Extract from existing wide format data
colleague_data <- get_colleague_format(full_results)
conc3_from_wide <- extract_concentration_from_wide(colleague_data$samples, "Conc_3")
head(conc3_from_wide)

# Quick plot of a concentration
show_plot_styles(full_results, "Conc_5")
plot_concentration(full_results, "Conc_1")
plot_concentration(full_results, "Conc_2")
plot_concentration(full_results, "Conc_3")
plot_concentration(full_results, "Conc_4")
plot_concentration(full_results, "Conc_5")
plot_concentration(full_results, "Conc_6")
plot_concentration(full_results, "Conc_7")
plot_concentration(full_results, "Conc_8")
plot_concentration(full_results, "Conc_9")
plot_concentration(full_results, "Conc_10")

plot_multiple_concentrations(full_results, c("Conc_1", "Conc_5", "Conc_10"))

# Check the untreated data structure
names(colleague_data$untreated)
head(colleague_data$untreated)
# Check the column order
names(colleague_data$samples)
